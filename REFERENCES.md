# References

**Free Fire OB54 — Standards, Frameworks, and Documentation**

---

## OWASP

| Reference | Description |
|-----------|-------------|
| [OWASP MASVS v2](https://mas.owasp.org/MASVS/) | Mobile Application Security Verification Standard |
| [OWASP MASTG v1](https://mas.owasp.org/MASTG/) | Mobile Application Security Testing Guide |
| [OWASP Mobile Top 10 2024](https://owasp.org/www-project-mobile-top-10/) | Top 10 Mobile Security Risks |
| [OWASP M1: Improper Platform Usage](https://mas.owasp.org/MASVS/activities/M1-Improper-Platform-Usage/) | Platform-specific security misuses |
| [OWASP M2: Insecure Data Storage](https://mas.owasp.org/MASVS/activities/M2-Insecure-Data-Storage/) | Insecure local data storage |
| [OWASP M3: Insecure Communication](https://mas.owasp.org/MASVS/activities/M3-Insecure-Communication/) | Insecure network communication |
| [OWASP M5: Insufficient Cryptography](https://mas.owasp.org/MASVS/activities/M5-Insufficient-Cryptography/) | Weak or insufficient cryptography |
| [OWASP M7: Client Code Quality](https://mas.owasp.org/MASVS/activities/M7-Client-Code-Quality/) | Code quality issues |

---

## CWE/SANS

| Reference | Description |
|-----------|-------------|
| [CWE-319](https://cwe.mitre.org/data/definitions/319.html) | Cleartext Transmission of Sensitive Information |
| [CWE-321](https://cwe.mitre.org/data/definitions/321.html) | Use of Hard-coded Cryptographic Key |
| [CWE-295](https://cwe.mitre.org/data/definitions/295.html) | Improper Certificate Validation |
| [CWE-494](https://cwe.mitre.org/data/definitions/494.html) | Download of Code Without Integrity Check |
| [CWE-798](https://cwe.mitre.org/data/definitions/798.html) | Use of Hard-coded Credentials |
| [CWE-294](https://cwe.mitre.org/data/definitions/294.html) | Authentication Bypass by Capture-replay |
| [CWE-353](https://cwe.mitre.org/data/definitions/353.html) | Missing Support for Integrity Check |
| [CWE-300](https://cwe.mitre.org/data/definitions/300.html) | Channel Accessible by Non-Endpoint |
| [CWE-312](https://cwe.mitre.org/data/definitions/312.html) | Cleartext Storage of Sensitive Information |
| [CWE-276](https://cwe.mitre.org/data/definitions/276.html) | Incorrect Default Permissions |
| [CWE-613](https://cwe.mitre.org/data/definitions/613.html) | Insufficient Session Expiration |
| [CWE-927](https://cwe.mitre.org/data/definitions/927.html) | Use of Implicit Intent for Sensitive Communication |
| [CWE-799](https://cwe.mitre.org/data/definitions/799.html) | Improper Control of Interaction Frequency |
| [CWE-290](https://cwe.mitre.org/data/definitions/290.html) | Authentication Bypass by Spoofing |
| [CWE-328](https://cwe.mitre.org/data/definitions/328.html) | Use of Weak Hash |
| [CWE-522](https://cwe.mitre.org/data/definitions/522.html) | Insufficiently Protected Credentials |
| [CWE-200](https://cwe.mitre.org/data/definitions/200.html) | Exposure of Sensitive Information |
| [CWE-749](https://cwe.mitre.org/data/definitions/749.html) | Exposed Dangerous Method or Function |
| [CWE-327](https://cwe.mitre.org/data/definitions/327.html) | Use of a Broken or Risky Cryptographic Algorithm |
| [CWE-345](https://cwe.mitre.org/data/definitions/345.html) | Insufficient Verification of Data Authenticity |
| [CWE-94](https://cwe.mitre.org/data/definitions/94.html) | Improper Control of Generation of Code |

---

## NIST

| Reference | Description |
|-----------|-------------|
| [NIST SP 800-38D](https://csrc.nist.gov/publications/detail/sp/800-38d/final) | Recommendations for Block Cipher Modes of Operation: Galois/Counter Mode (GCM) |
| [NIST SP 800-57](https://csrc.nist.gov/publications/detail/sp/800-57-part-1/rev-5/final) | Recommendation for Key Management |
| [NIST SP 800-175B](https://csrc.nist.gov/publications/detail/sp/800-175b/rev-1/final) | Guideline for Using Cryptographic Standards |

---

## RFCs

| Reference | Description |
|-----------|-------------|
| [RFC 7748](https://datatracker.ietf.org/doc/html/rfc7748) | Elliptic Curves for Diffie-Hellman Key Agreement |
| [RFC 8446](https://datatracker.ietf.org/doc/html/rfc8446) | The Transport Layer Security (TLS) Protocol Version 1.3 |
| [RFC 5246](https://datatracker.ietf.org/doc/html/rfc5246) | The Transport Layer Security (TLS) Protocol Version 1.2 |
| [RFC 4493](https://datatracker.ietf.org/doc/html/rfc4493) | The AES-CMAC Algorithm |
| [RFC 3394](https://datatracker.ietf.org/doc/html/rfc3394) | Advanced Encryption Standard (AES) Key Wrap Algorithm |
| [RFC 7296](https://datatracker.ietf.org/doc/html/rfc7296) | Internet Key Exchange Protocol Version 2 (IKEv2) |

---

## Android Security

| Reference | Description |
|-----------|-------------|
| [Android Network Security Config](https://developer.android.com/training/articles/security-config) | Network security configuration documentation |
| [Android Keystore System](https://developer.android.com/training/articles/keystore) | Hardware-backed key storage |
| [EncryptedSharedPreferences](https://developer.android.com/reference/androidx/security/crypto/EncryptedSharedPreferences) | Encrypted preference storage |
| [FileProvider](https://developer.android.com/reference/androidx/core/content/FileProvider) | Secure file sharing between apps |
| [Play Integrity API](https://developer.android.com/google/play/integrity) | Device and app integrity verification |
| [SafetyNet Attestation](https://developer.android.com/training/safetynet/attestation) | Device attestation (deprecated, replaced by Play Integrity) |
| [WebView Security](https://developer.android.com/develop/ui/views/layout/webapps/managing-webview) | WebView security best practices |

---

## CAPEC

| Reference | Description |
|-----------|-------------|
| [CAPEC-46](https://capec.mitre.org/data/definitions/46.html) | Encryption Brute Force |
| [CAPEC-159](https://capec.mitre.org/data/definitions/159.html) | Intercept Communication |

---

## Cryptography

| Reference | Description |
|-----------|-------------|
| [NIST FIPS 197](https://csrc.nist.gov/publications/detail/fips/197/final) | Advanced Encryption Standard (AES) |
| [NIST FIPS 800-38A](https://csrc.nist.gov/publications/detail/sp/800-38a/final) | Block Cipher Modes of Operation |
| [BouncyCastle Documentation](https://www.bouncycastle.org/) | Cryptographic library documentation |

---

*References version: 2.0 · Last updated: July 2026*
